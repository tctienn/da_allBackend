...
public class InvoiceService {
    ...
    ///// hàm dành riêng cho grapservice (microservice)
    // hàm này đang dùng cho giao tiếp đồng bộ của service nhân viên
    public  InvoiceEntity getInvoiceBySoHD(String soHDPrams){
        InvoiceEntity invoice =  invoiceRepository.getInvoiceWaitsBySoHD(soHDPrams).orElse(null);
        return invoice;
    }
    /// hàm này dùng để truy vấn lấy thông tin chi tiết hóa đơn
    public InvoiceEntity getInvoiceBySoHDs(String soHD){
        return invoiceRepository.findBySohd(soHD);
    }

    @Transactional
    public void updateStatusInvoice(String soHDParam, String statusParam){
         InvoiceEntity invoiceEntity = invoiceRepository.findBySohd(soHDParam);

        invoiceEntity.setGiaohang(statusParam);
    }
    ...
    ////// kết thúc khu vực hàm dành riêng cho grapservice
